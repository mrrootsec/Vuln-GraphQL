...
const resolvers = {
  Query: {
    ...
    rsvpedUsers: (_, { event_id }, { db }) => 
      ...
      healthCheck: async (_, { input }) => {
        // Weak validation: Allow only alphanumeric characters and spaces
        const sanitizedInput = input.replace(/[^a-zA-Z0-9 |]/g, '');
  
        // Append allowed command if input passes validation
        const command = `uptime && date ${sanitizedInput}`.trim();
  
        return new Promise((resolve, reject) => {
          exec(command, (error, stdout, stderr) => {
            if (error) {
              reject(`Error: Please provide valid input}`);
            } else {
              resolve(stdout.trim());
            }
          });
